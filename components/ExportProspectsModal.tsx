...
// Column keys are the API contract. Labels resolved at render via
// useTranslations('components.exportProspectsModal.columns').
const COLUMN_KEYS = [
  'email', 'first_name', 'last_name', 'company', 'title', 'linkedin_url',
  'website', 'status', 'source', 'tags', 'notes', 'added_at',
] as const
...
export function ExportProspectsModal({ isOpen, onClose, selectedIds, totalCount, filteredCount, filters }: Props) {
  const t = useTranslations('components.exportProspectsModal')
  const tColumns = useTranslations('components.exportProspectsModal.columns')
  ...
  async function handleExport() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setError(err.error ?? t('errorFallback', { status: res.status }))
        return
      }
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between">
          <h2 id="export-modal-title" className="text-base font-bold text-[#1a1a2e]">{t('title')}</h2>
        ...
        <div>
          <p id="format-label" className="text-xs font-semibold text-[#6b5e4e] mb-2 uppercase tracking-wide">{t('formatLabel')}</p>
          <div role="radiogroup" aria-labelledby="format-label" className="flex gap-2">
            {(['csv', 'xlsx'] as const).map(f => (
              ...
              >
                {f === 'csv' ? t('formatCsv') : t('formatXlsx')}
              </button>
            ))}
          </div>
        </div>
        ...
        <fieldset>
          ...
          <p aria-hidden="true" className="text-xs font-semibold text-[#6b5e4e] mb-2 uppercase tracking-wide">{t('whatToExport')}</p>
          <div className="flex flex-col gap-1.5">
            {selectedIds.length > 0 && (
              <label className="flex items-center gap-2.5 cursor-pointer">
                ...
                <span className="text-sm text-[#1a1a2e]">{t('scopeSelected')} <span className="text-[#8a7e6e]">{t('scopeSelectedCount', { count: selectedIds.length })}</span></span>
              </label>
            )}
            <label className="flex items-center gap-2.5 cursor-pointer">
              <input type="radio" name="scope" checked={scope === 'filtered'} onChange={() => setScope('filtered')} className="accent-[#3b6bef]" />
              <span className="text-sm text-[#1a1a2e]">{t('scopeFiltered')} <span className="text-[#8a7e6e]">{t('scopeFilteredCount', { count: filteredCount })}</span></span>
            </label>
            <label className="flex items-center gap-2.5 cursor-pointer">
              <input type="radio" name="scope" checked={scope === 'all'} onChange={() => setScope('all')} className="accent-[#3b6bef]" />
              <span className="text-sm text-[#1a1a2e]">{t('scopeAll')} <span className="text-[#8a7e6e]">{t('scopeAllCount', { count: totalCount })}</span></span>
            </label>
          </div>
        </fieldset>
        ...
        <fieldset>
          <legend className="sr-only">{t('columnsLegend')}</legend>
          <p aria-hidden="true" className="text-xs font-semibold text-[#6b5e4e] mb-2 uppercase tracking-wide">{t('columnsShort')}</p>
          <div className="grid grid-cols-2 gap-1.5">
            {COLUMN_KEYS.map(key => (
              <label key={key} className="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" checked={selectedCols.has(key)} onChange={() => toggleCol(key)} className="accent-[#3b6bef] w-3.5 h-3.5" />
                <span className="text-sm text-[#1a1a2e]">{tColumns(key)}</span>
              </label>
            ))}
          </div>
        </fieldset>
        ...
        <div className="flex gap-2 pt-1">
          ...
          >
            {exporting ? t('exporting') : t('exportCta', { count: exportCount })}
          </button>
        </div>
      </div>
    </div>
