...
export function Tooltip({ content, children, placement = 'top' }: Props) {
  ...
  return (
    ...
    >
      {children}
      {visible && content && (
        <div className={`absolute ${box} z-50 max-w-xs bg-gray-900 text-white text-xs px-3 py-2 rounded-md shadow-lg pointer-events-none animate-in fade-in duration-150`}>
          {content}
          <div className={arrow} />
        </div>
      )}
