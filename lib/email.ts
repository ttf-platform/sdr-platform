/**
 * Centralized Resend client (singleton) + transactional helpers.
 *
 * Created during Sprint Widget Help. Currently used by:
 *   - app/api/bot/escalate (admin notification)
 *   - app/api/bot/message (auto-escalation triggers admin email)
 *   - app/api/bug-reports (critical bug → admin email)
 *
 * The 2 pre-existing Resend usages (app/api/team/invite, app/api/admin/broadcast)
 * will be migrated to this singleton in a dedicated post-launch refactor.
 *
 * Environment variables:
 *   RESEND_API_KEY   — required
 *   ADMIN_NOTIFICATION_EMAIL  — destination for support escalations (currently cyrus@noos.fr)
 */
...
/**
 * Send an escalation notification to the admin. Non-blocking — errors are
 * logged but do not throw, since the escalation row is already saved in DB.
 */
export async function sendAdminEscalationEmail(params: {
  ...
  console.log('[email] ADMIN_NOTIFICATION_EMAIL =', adminEmail ?? '(not set)');
  if (!adminEmail) {
    console.warn('[email] ADMIN_NOTIFICATION_EMAIL not set, skipping escalation email');
...
`.trim();
  ...
  console.log('[email] sending escalation', { to: adminEmail, from: FROM_ADDRESS, escalationId: params.escalationId });
  try {
...
/**
 * Send a critical bug notification to the admin.
 */
