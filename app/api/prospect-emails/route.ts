...
export async function GET(request: Request) {
  ...
  //
  // .eq('origin','campaign') scopes the campaign Emails tab to rows
  // generated by draft-generation.ts / variant convergence. Inbox-reply
  // copies (migration 088) belong to the inbox thread view, not to this
  // list — surfacing them here would render each reply as a spurious
  // extra "sent" line on the campaign's Emails tab.
  let q = admin
  ...
  // Same origin='campaign' scope as the main query above so the badges
  // reflect the same set of rows the user sees in the list.
