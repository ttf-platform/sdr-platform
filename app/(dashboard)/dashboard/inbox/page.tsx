        ...
        <div className="flex gap-1 border border-[#e8e3dc] rounded-xl p-1 bg-white">
          {(['inbox','starred','archive'] as const).map(t => (
            <button key={t} onClick={() => setTab(t)}
              className={"px-4 py-1.5 rounded-lg text-sm font-medium capitalize transition-colors " + (tab === t ? 'bg-white border border-[#e8e3dc] text-[#3b6bef] font-semibold shadow-sm' : 'text-[#8a7e6e]')}>
              {t === 'starred' ? '⭐ Starred' : t.charAt(0).toUpperCase() + t.slice(1)}
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-white border border-[#e8e3dc] rounded-xl overflow-hidden">
          {filtered.length === 0 ? (
            ...
            <div key={m.id} onClick={() => { setSelected(m); setAiDraft('') }}
              className={"flex items-start gap-3 px-4 py-3 border-b border-[#f0ece6] cursor-pointer hover:bg-[#faf8f5] " + (selected?.id === m.id ? 'bg-[#f7f8ff] border-l-2 border-l-[#3b6bef]' : '') + (!m.is_read ? ' font-medium' : '')}>
              <div className="w-8 h-8 rounded-full bg-gradient-to-br from-[#3b6bef] to-[#8b5cf6] flex items-center justify-center text-white text-xs font-bold flex-shrink-0">
                {(m.from_name?.[0] || m.from_email?.[0] || '?').toUpperCase()}
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between">
                  <span className={"text-sm truncate " + (!m.is_read ? 'font-semibold text-[#1a1a2e]' : 'text-[#1a1a2e]')}>{m.from_name || m.from_email}</span>
                  <span className="text-xs text-[#8a7e6e] flex-shrink-0 ml-2">{new Date(m.received_at).toLocaleDateString()}</span>
                </div>
                {m.subject && <div className="text-xs text-[#6b5e4e] truncate">{m.subject}</div>}
                <div className="text-xs text-[#8a7e6e] truncate">{m.body?.slice(0, 60)}...</div>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="bg-white border border-[#e8e3dc] rounded-xl overflow-hidden">
          {!selected ? (
            <div className="p-12 text-center">
              <div className="text-4xl mb-3">📭</div>
              <div className="font-bold text-[#1a1a2e] mb-1">Select a message</div>
              <div className="text-sm text-[#8a7e6e]">Click a message on the left to read it.</div>
            </div>
          ) : (
            <div className="flex flex-col h-full">
              <div className="flex items-start justify-between p-4 border-b border-[#f0ece6]">
                <div>
              ...
              <div className="p-4 flex-1 overflow-y-auto">
                <p className="text-sm text-[#4a3f32] leading-relaxed whitespace-pre-wrap">{selected.body}</p>
              </div>
