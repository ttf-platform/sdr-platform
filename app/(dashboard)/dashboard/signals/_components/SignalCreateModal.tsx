...
export function SignalCreateModal({
  ...
  const t = useTranslations('dashboard.signals.create')
  ...
  function selectTemplate(tpl: TemplateId) {
    ...
    // Default signal name reuses the same i18n label displayed on the picker
    // card so the two stay in lockstep automatically.
    const labelKeyByTemplate: Record<TemplateId, 'templateHiringLabel' | 'templateFundingLabel' | 'templateTechLabel'> = {
      hiring_role:       'templateHiringLabel',
      recent_funding:    'templateFundingLabel',
      tech_stack_change: 'templateTechLabel',
    }
    setSignalName(t(labelKeyByTemplate[tpl]))
    setTemplateStep('form')
  }
  ...
  async function saveTemplate() {
    ...
    if (res.signal) {
      ...
      setError(res.error ?? t('errorSaveSignal'))
      setSaving(false)
    }
  }
  ...
  function getTitle() {
    if (mode === null) return t('titleCreate')
    if (mode === 'template' && templateStep === 'form') {
      const titleKeyByTemplate: Record<TemplateId, 'titleHiringRoleForm' | 'titleRecentFundingForm' | 'titleTechStackChangeForm'> = {
        hiring_role:       'titleHiringRoleForm',
        recent_funding:    'titleRecentFundingForm',
        tech_stack_change: 'titleTechStackChangeForm',
      }
      return selectedTemplate ? t(titleKeyByTemplate[selectedTemplate]) : t('titleConfigureSignal')
    }
    if (mode === 'custom' && customStep === 'describe') return t('titleCustomSignal')
    if (mode === 'custom' && customStep === 'building') return t('titleAnalyzing')
    if (mode === 'custom' && customStep === 'preview') return t('titleAiReady')
  ...
  function renderContent() {
    // ── Mode picker ──────────────────────────────────────────────────────────
    if (mode === null) {
      const templateCards: Array<{ id: TemplateId; emoji: string; labelKey: 'templateHiringLabel' | 'templateFundingLabel' | 'templateTechLabel'; descKey: 'templateHiringDesc' | 'templateFundingDesc' | 'templateTechDesc' }> = [
        { id: 'hiring_role',       emoji: '💼', labelKey: 'templateHiringLabel', descKey: 'templateHiringDesc' },
        { id: 'recent_funding',    emoji: '💰', labelKey: 'templateFundingLabel', descKey: 'templateFundingDesc' },
        { id: 'tech_stack_change', emoji: '🔧', labelKey: 'templateTechLabel', descKey: 'templateTechDesc' },
      ]
      return (
        <div className="flex flex-col gap-5">
          <p className="text-sm text-[#8a7e6e]">{t('pickerPrompt')}</p>
          ...
          <div>
            <div className="flex items-center gap-2 mb-3">
              ...
              <div>
                <p className="text-sm font-semibold text-[#1a1a2e]">{t('useTemplateTitle')}</p>
                <p className="text-xs text-[#8a7e6e]">{t('useTemplateDescription')}</p>
              </div>
            </div>
            <div className="grid grid-cols-3 gap-2">
              {templateCards.map(card => (
                ...
                >
                  ...
                  <div className="text-xs font-semibold text-[#3b6bef] mb-1 leading-snug">{t(card.labelKey)}</div>
                  <div className="text-[11px] text-[#8a7e6e] leading-snug">{t(card.descKey)}</div>
                </button>
              ))}
            </div>
          </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="font-semibold text-sm text-[#1a1a2e] group-hover:text-[#3b6bef]">{t('customTitle')}</span>
                  <span className="bg-[#eef1fd] text-[#3b6bef] text-[10px] font-semibold px-2 py-0.5 rounded-full border border-[#dde6fd]">
                    {t('customBadge')}
                  </span>
                </div>
                <p className="text-xs text-[#8a7e6e]">{t('customDescription')}</p>
              </div>
    ...
    if (mode === 'template' && templateStep === 'form') {
      return (
        <div className="flex flex-col gap-4">
          <div>
            <label className="text-xs font-semibold text-[#6b5e4e] mb-1 block">{t('signalNameLabel')}</label>
            <input
              type="text"
              value={signalName}
            ...
            <>
              <div>
                <label className="text-xs font-semibold text-[#6b5e4e] mb-1 block">
                  {t('roleToMonitorLabel')} <span className="text-red-400">*</span>
                </label>
                <input
                  type="text"
                  value={roleText}
              ...
              <div>
                <label className="text-xs font-semibold text-[#6b5e4e] mb-2 block">{t('companySizeLabel')}</label>
          ...
          {selectedTemplate === 'recent_funding' && (
            <>
              <div>
                <label className="text-xs font-semibold text-[#6b5e4e] mb-2 block">{t('fundingStageLabel')}</label>
                <div className="flex flex-wrap gap-2">
              ...
              <div>
                <label className="text-xs font-semibold text-[#6b5e4e] mb-1 block">{t('minAmountLabel')}</label>
                <input
                  type="text"
                  value={minFunding}
          ...
          {selectedTemplate === 'tech_stack_change' && (
            <>
              <div>
                <label className="text-xs font-semibold text-[#6b5e4e] mb-1 block">
                  {t('toolsToMonitorLabel')} <span className="text-red-400">*</span>
                </label>
                <input
                  type="text"
                  value={techTools}
                  ...
                  className="w-full border border-[#e8e3dc] rounded-lg px-3 py-2 text-sm focus-visible:outline-none focus-visible:border-[#3b6bef]"
                />
                <p className="text-[11px] text-[#8a7e6e] mt-1">{t('toolsHelp')}</p>
              </div>
              <div>
                <label className="text-xs font-semibold text-[#6b5e4e] mb-2 block">{t('eventTypeLabel')}</label>
                <div className="flex gap-2">
                  {TECH_EVENT_OPTS.map(e => (
                    ...
                    >
                      {t(`event_${e}` as 'event_installed' | 'event_uninstalled' | 'event_both')}
                    </button>
                  ))}
                </div>
              </div>
    ...
    if (mode === 'custom' && customStep === 'building') {
      return (
        <div className="py-8 text-center flex flex-col items-center gap-3">
          ...
          <p className="text-sm font-medium text-[#1a1a2e]">{t('analyzingHeading')}</p>
          <p className="text-xs text-[#8a7e6e]">{t('analyzingSubtext')}</p>
        </div>
      )
    }
    ...
    if (mode === 'custom' && customStep === 'preview' && buildResult) {
      if (!buildResult.feasible) {
        const notFeasibleItemKeys: Array<'notFeasibleItem0' | 'notFeasibleItem1' | 'notFeasibleItem2' | 'notFeasibleItem3' | 'notFeasibleItem4' | 'notFeasibleItem5'> = [
          'notFeasibleItem0', 'notFeasibleItem1', 'notFeasibleItem2',
          'notFeasibleItem3', 'notFeasibleItem4', 'notFeasibleItem5',
        ]
        return (
          <div className="flex flex-col gap-4">
            <div className="bg-red-50 border border-red-200 rounded-xl p-4">
              <p className="text-red-500 font-semibold text-sm mb-2">✗ {t('notFeasibleHeading')}</p>
            ...
            <div className="bg-[#f7f8ff] border border-[#dde6fd] rounded-xl p-4">
              <p className="text-xs font-semibold text-[#3b6bef] mb-2">{t('notFeasibleSuggestionHeading')}</p>
              <ul className="space-y-1">
                {notFeasibleItemKeys.map(k => (
                  <li key={k} className="text-xs text-[#6b5e4e] flex items-start gap-1.5">
      ...
      return (
        <div className="flex flex-col gap-4">
          <div className="bg-green-50 border border-green-200 rounded-xl px-4 py-3 flex items-center gap-2">
            <span className="text-green-600 font-semibold text-sm">✓ {t('feasibleHeading')}</span>
          </div>
          ...
          <div>
            <label className="text-xs font-semibold text-[#6b5e4e] mb-1 block">{t('previewDescriptionLabel')}</label>
            <textarea
          ...
          <div className="bg-[#f7f8ff] border border-[#dde6fd] rounded-xl p-4">
            <p className="text-xs font-semibold text-[#3b6bef] mb-3">{t('monitoringHeading')}</p>
            <div className="flex flex-col gap-1.5 text-xs text-[#4a4a5a]">
              {!!cfg.source && (
                <div className="flex gap-2">
                  <span className="text-[#8a7e6e] w-20 flex-shrink-0">{t('monitoringSourceLabel')}</span>
                  <span className="font-medium">{String(cfg.source).replace(/_/g, ' ')}</span>
                </div>
              )}
              {!!cfg.search_strategy && (
                <div className="flex gap-2">
                  <span className="text-[#8a7e6e] w-20 flex-shrink-0">{t('monitoringStrategyLabel')}</span>
              ...
              {Array.isArray(cfg.match_keywords) && cfg.match_keywords.length > 0 && (
                <div className="flex gap-2">
                  <span className="text-[#8a7e6e] w-20 flex-shrink-0">{t('monitoringKeywordsLabel')}</span>
                  <span>{(cfg.match_keywords as string[]).slice(0, 5).join(', ')}{(cfg.match_keywords as string[]).length > 5 ? ` ${t('keywordsMore', { count: (cfg.match_keywords as string[]).length - 5 })}` : ''}</span>
                </div>
              )}
              {typeof cfg.freshness_days === 'number' && (
                <div className="flex gap-2">
                  <span className="text-[#8a7e6e] w-20 flex-shrink-0">{t('monitoringFreshnessLabel')}</span>
                  <span>{t('freshnessDays', { days: cfg.freshness_days })}</span>
                </div>
              )}
            </div>
          </div>
    ...
    if (mode === 'template' && templateStep === 'form') {
      ...
      return (
        <div className="flex gap-3 w-full">
          ...
          >
            {saving ? tCommon('saving') : t('saveSignal')}
          </button>
        </div>
      )
    }
